...
const LayerRow = styled.div<ILayerRow>`
  ...
  span {
    ...
    font-weight: ${(props: ILayerRow) => (props.hasChildren ? "700" : "400")};
    letter-spacing: ${(props: ILayerRow) =>
      props.hasChildren ? "1.5px" : "0.6px"};
    line-height: ${(props: ILayerRow) => (props.hasChildren ? "20px" : "18px")};
    font-size: ${(props: ILayerRow) => (props.hasChildren ? "10px" : "13px")};
      ...
      <SidebarContainer visible={visible}>
        <SidebarContent>
          ...
          {sortedLayers.map((c, i) => {
            if (!hasChildren(c) && c.treePath && c.treePath.length === 1)
              return null;
            return (
              <>
                <LayerRow
