...
const compareBox3 = (box1: THREE.Box3, box2: THREE.Box3): boolean => {
  ...
  const widthDiff = Math.abs(
    box1.max.x - box1.min.x - (box2.max.x - box2.min.x)
  );
  const heightDiff = Math.abs(
    box1.max.y - box1.min.y - (box2.max.y - box2.min.y)
  );
...
export function Bounds({
    ...
    return {
      ...
      refresh() {
        ...
        if (ref.current && ref.current.children.length > 0) {
          ...
          const visualizationsGroup =
            targetGroup.children[2] || new THREE.Group();
          ...
          [...mapGroup.children, ...visualizationsGroup.children].forEach(
      ...
      fit(boxToFit?: THREE.Box3) {
        ...
        if (controls) {
          ...
          controls.rotate?.(
            getAbsoluteAngle(0, controls.azimuthAngle || 0),
            -getAbsoluteAngle(-Math.PI, controls.polarAngle || 0),
            true
          );

          controls
            .fitToBox?.(newBox, true, {
              cover: false,
              paddingTop: 0.3,
              paddingBottom: 0.3,
              paddingLeft: 0.3,
  ...
  React.useEffect(() => {
    ...
    return () => {
      ...
      scene.removeEventListener("recenter", () => {
        api.fit();
      });
